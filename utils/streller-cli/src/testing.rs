...
fn run_unit_tests() {
    ...
    match choice {
        ...
        Ok("Run with verbose output") => {
            execute_command(
                "cargo",
                &["test", "--workspace", "--exclude", "e2e-tests", "--verbose"],
            );
        }
        Ok("Run with release optimizations") => {
            execute_command(
                "cargo",
                &["test", "--workspace", "--exclude", "e2e-tests", "--release"],
            );
        }
        _ => {}
    }
...
fn network_connectivity_tests() {
    ...
    execute_command(
        ...
        &[
            ...
            "deploy",
            "--wasm",
            "target/release/contract_example.wasm",
            "--source",
            "test_account",
        ],
    );
...
fn test_coverage() {
    ...
    execute_command(
        "cargo",
        &["tarpaulin", "--workspace", "--exclude", "e2e-tests", "--out", "Html"],
    );
...
fn test_analysis() {
    ...
    // Analyze test output
    let _test_files = ["./target/debug/.fingerprint/", "./target/release/.fingerprint/"];
...
fn check_test_prerequisites() -> bool {
    ...
    if fs::metadata("./target/release").is_err() {
