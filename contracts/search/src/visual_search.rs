...
impl VisualSearch {
    ...
    pub fn store_visual_metadata(env: &Env, content_id: String, metadata: VisualMetadata) {
    ...
    pub fn get_visual_metadata(env: &Env, content_id: String) -> Option<VisualMetadata> {
    ...
    pub fn store_similar_content(env: &Env, content_id: String, similar_ids: Vec<String>) {
        let cache_key = Self::similar_cache_key(env, &content_id);
    ...
    pub fn find_by_object(env: &Env, object_type: String) -> Vec<String> {
    ...
    pub fn calculate_visual_score(
        ...
        let color_score =
            Self::calculate_color_match_score(&metadata.dominant_colors, &query_colors);
        score += color_score;

        // Object matching (0-600 points)
        let object_score =
            Self::calculate_object_match_score(&metadata.detected_objects, &query_objects);
        score += object_score;
    ...
    pub fn get_thumbnail(env: &Env, content_id: String) -> Option<String> {
    ...
    pub fn get_aspect_ratio(env: &Env, content_id: String) -> u32 {
        if let Some(metadata) = Self::get_visual_metadata(env, content_id) {
    ...
    pub fn is_high_quality(env: &Env, content_id: String, threshold: u32) -> bool {
        if let Some(metadata) = Self::get_visual_metadata(env, content_id) {
    ...
    pub fn get_color_palette(env: &Env, content_id: String) -> Vec<String> {
        if let Some(metadata) = Self::get_visual_metadata(env, content_id) {
    ...
    pub fn get_detected_objects(env: &Env, content_id: String) -> Vec<String> {
        if let Some(metadata) = Self::get_visual_metadata(env, content_id) {
