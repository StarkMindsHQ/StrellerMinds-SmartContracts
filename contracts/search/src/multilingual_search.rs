...
impl MultilingualSearch {
    ...
    pub fn get_multilingual_content(
        env: &Env,
        content_id: String,
    ) -> Option<MultilingualContent> {
        let key = DataKey::MultilingualContent(content_id);
        env.storage().persistent().get(&key)
    }
    
    /// Store user's language preferences
    pub fn store_language_preferences(
        env: &Env,
        user: Address,
    ...
    pub fn get_language_preferences(
        env: &Env,
        user: Address,
    ) -> Option<LanguagePreferences> {
    ...
    pub fn search_by_language(
        ...
        let content_ids = env.storage().persistent()
        ...
        if let Some(prefs) = Self::get_language_preferences(env, user) {
            for i in 0..content_ids.len() {
                if let Some(content_id) = content_ids.get(i) {
                    if Self::is_available_in_language(env, content_id.clone(), prefs.preferred_language.clone()) {
    ...
    pub fn is_available_in_language(
        env: &Env,
        content_id: String,
        language: Language,
    ) -> bool {
    ...
    pub fn filter_by_translation_quality(
        ...
        for i in 0..content_ids.len() {
            if let Some(content_id) = content_ids.get(i) {
                let quality = Self::get_translation_quality(env, content_id.clone(), language.clone());
    ...
    pub fn get_language_content_count(
        env: &Env,
        language: Language,
    ) -> u32 {
        let lang_key = Self::language_index_key(env, &language);
        
        if let Some(content_ids) = env.storage().persistent()
            .get::<String, Vec<String>>(&lang_key) {
            content_ids.len()
        } else {
            0
    ...
    pub fn detect_query_language(
        env: &Env,
        query: String,
    ) -> Language {
        // Emit event for off-chain language detection
        env.events().publish(
            (soroban_sdk::symbol_short!("detect_ln"),),
            query
        );
