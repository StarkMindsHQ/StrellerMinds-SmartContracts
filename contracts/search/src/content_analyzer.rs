...
impl ContentAnalyzer {
    ...
    pub fn store_analysis(env: &Env, content_id: String, analysis: ContentAnalysis) {
    ...
    pub fn find_by_tag(env: &Env, tag: String) -> Vec<String> {
        let tag_key = Self::tag_index_key(env, &tag);
        env.storage()
            .persistent()
            .get::<String, Vec<String>>(&tag_key)
    ...
    pub fn find_by_skill(env: &Env, skill_name: String) -> Vec<String> {
    ...
    pub fn get_tags(env: &Env, content_id: String) -> Vec<String> {
    ...
    pub fn get_skills(env: &Env, content_id: String) -> Vec<Skill> {
        if let Some(analysis) = Self::get_analysis(env, content_id) {
    ...
    pub fn get_topics(env: &Env, content_id: String) -> Vec<Topic> {
        if let Some(analysis) = Self::get_analysis(env, content_id) {
    ...
    pub fn get_quality_score(env: &Env, content_id: String) -> u32 {
        if let Some(analysis) = Self::get_analysis(env, content_id) {
    ...
    pub fn get_difficulty_score(env: &Env, content_id: String) -> u32 {
        if let Some(analysis) = Self::get_analysis(env, content_id) {
    ...
    pub fn calculate_content_similarity(env: &Env, content_a: String, content_b: String) -> u32 {
        ...
        // Compare tags
        let tag_similarity =
            Self::calculate_tag_similarity(&a.auto_generated_tags, &b.auto_generated_tags);
        similarity += tag_similarity;

        // Compare topics
        let topic_similarity =
            Self::calculate_topic_similarity(&a.extracted_topics, &b.extracted_topics);
                ...
                for j in 0..topics_b.len() {
                    if let Some(topic_b) = topics_b.get(j) {
                        if topic_a.name == topic_b.name {
                            ...
                            let avg_relevance =
                                (topic_a.relevance_score + topic_b.relevance_score) / 2;
