...
impl VoiceSearch {
    ...
    pub fn get_voice_query(env: &Env, timestamp: u64) -> Option<ProcessedVoiceQuery> {
        let key = Self::voice_query_key(env, timestamp);
        env.storage().persistent().get(&key)
    }

    /// Create or update conversation session
    pub fn create_conversation_session(env: &Env, user: Address) -> String {
    ...
    pub fn get_conversation_session(env: &Env, session_id: String) -> Option<ConversationSession> {
    ...
    fn add_to_conversation(env: &Env, user: Address, query: &ProcessedVoiceQuery) {
    ...
    pub fn process_conversational_query(
        ...
        let context_strings =
            if let Some(session) = Self::get_user_active_session(env, user.clone()) {
    ...
    pub fn handle_follow_up(env: &Env, user: Address, follow_up_text: String) -> ProcessedQuery {
    ...
    pub fn get_confidence_score(env: &Env, timestamp: u64) -> u32 {
    ...
    pub fn get_context_summary(env: &Env, user: Address) -> Vec<String> {
    ...
    pub fn clear_context(env: &Env, user: Address) {
        if let Some(mut session) = Self::get_user_active_session(env, user.clone()) {
    ...
    pub fn get_voice_quality_metrics(env: &Env, timestamp: u64) -> VoiceQualityMetrics {
        if let Some(query) = Self::get_voice_query(env, timestamp) {
            VoiceQualityMetrics {
