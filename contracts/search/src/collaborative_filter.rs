...
impl CollaborativeFilter {
    ...
    pub fn store_similarity(env: &Env, user_a: Address, user_b: Address, score: SimilarityScore) {
    ...
    pub fn get_similarity(env: &Env, user_a: Address, user_b: Address) -> Option<SimilarityScore> {
    ...
    pub fn calculate_collaborative_score(env: &Env, user: Address, content_id: String) -> u32 {
        // Get similar users
        ...
        for i in 0..similar_users.len() {
            if let Some(similar_user) = similar_users.get(i) {
                ...
                if let Some(sim_score) =
                    Self::get_similarity(env, user.clone(), similar_user.clone())
                {
                    // Check if similar user engaged with content
                    if let Some(interaction_score) =
                        Self::get_user_content_interaction(env, similar_user, content_id.clone())
                    {
    ...
    pub fn record_interaction(env: &Env, user: Address, interaction: UserInteraction) {
        // Get existing interactions
        let key = DataKey::UserInteractions(user.clone());
        let mut interactions = env
            .storage()
            .persistent()
    ...
    pub fn get_user_interactions(env: &Env, user: Address) -> Vec<UserInteraction> {
    ...
    pub fn calculate_user_similarity(env: &Env, user_a: Address, user_b: Address) -> u32 {
        ...
        let jaccard = if total_unique > 0 {
            (overlap * 100).checked_div(total_unique).unwrap_or(0)
        } else {
            0
        };
        ...
        let weighted_avg =
            if overlap > 0 { overlap_weight.checked_div(overlap).unwrap_or(0) } else { 0 };
