...
async fn test_load_diagnostics_overhead() -> Result<()> {
    let harness = setup_test_harness!();
    let diagnostics_id = harness.get_contract_id("diagnostics").expect("Diagnostics contract not found");
    ...
    for i in 0..20 {
        // Mock operation
        harness.client.invoke_contract(token_id, "mint", &[format!("--to user_1"), format!("--amount {}", i)], "admin").await?;
    }
    ...
    let overhead = (monitored_duration.as_secs_f64() / baseline_duration.as_secs_f64() - 1.0) * 100.0;
